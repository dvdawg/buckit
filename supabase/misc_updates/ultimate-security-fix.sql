...
CREATE POLICY "buckets_auth_only" ON buckets
    FOR ALL USING (auth.uid() IS NOT NULL);

CREATE POLICY "items_auth_only" ON items
    FOR ALL USING (auth.uid() IS NOT NULL);

CREATE POLICY "bucket_collaborators_auth_only" ON bucket_collaborators
    FOR ALL USING (auth.uid() IS NOT NULL);

CREATE POLICY "completions_auth_only" ON completions
    FOR ALL USING (auth.uid() IS NOT NULL);

CREATE POLICY "friendships_auth_only" ON friendships
    FOR ALL USING (auth.uid() IS NOT NULL);
